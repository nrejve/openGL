...
void update(int value) {
  ...
  if (b <= -10.0)  // moving of run way
    b = 0.0;

  glutPostRedisplay();
  glutTimerFunc(5, update, 0);  // delay
}
...
void display(void) {
  ...
  if (flag == 0) {
    ...
    glTranslated(a, c, 0.0);
    glColor3f(1.0, 0.0, 0.0);
    glBegin(GL_POLYGON);  // vertical stabilizer
    ...
    if (c > 400)  // timer to jump to 3rd display
    {
      ...
      if (e > 350)  // timer to call blast function
      {
        blast();
        e = 600;
      }
...
void keys(unsigned char key, int x, int y) {
  ...
  if (key == 'm') {
    a += 15.0;  // Plane position takeoff on x axis
    b -= 5.0;  // Road Strip backward movement
    c += 15.0;  // take off at certain angle on y axis
